...
import fashionLoader from '../../assets/lottie/elecrtonic.json'; // لو مسار اللودر غير كده عدله هنا
...
export default function Electronic() {
  ...
  const [pageLoading, setPageLoading] = useState(true); // لودر الصفحة كلها
  ...
  useEffect(() => {
    setLoading(true);
    axios.get('https://lavender-eel-222276.hostingersite.com/api/items/category/3')
      .then(response => {
        ...
        setLoading(false);
        setTimeout(() => setPageLoading(false), 1500); // علشان ندي فرصة للودر يظهر شوية
      })
      .catch(err => {
        console.error(err);
        setLoading(false);
